...
def generate_vd(text, caption, num_steps, cfg_t, cfg_c, seed_raw, speed, dict_text, save_audio):
    if not text or not text.strip():
        raise gr.Error("テキストを入力してください")
    original = text.strip()
    processed = apply_dict(original, dict_text)
...
def build_ui():
    ...
    with gr.Blocks(title="Irodori-TTS ゆうぷろカスタム V1.4.0 [ローカル版]", theme=gr.themes.Soft()) as app:
        ...
        v_btn.click(
            generate_vd,
            [v_text, v_cap, v_steps, v_cfg_t, v_cfg_c, v_seed, v_speed, dict_input, v_save],
            [v_out, v_info],
        )
